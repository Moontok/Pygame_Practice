...
def menu_loop(screen, game_manager):
    ...
    main_menu = pg.transform.scale(main_menu, game_manager.get_dimensions())
    ...
    while game_manager.in_main_menu:
        ...
        display_text(
            ...
            screen,
            pg.Color("white"),
            (game_manager.window_width * .1, game_manager.window_height * .78)
        )
...
def game_loop(screen, game_manager):
    ...
    background = pg.transform.scale(background, game_manager.get_dimensions())

    snake = Snake(game_manager.center(), game_manager.segment_size)
    snake.add_segment()
    food = Food(0, 0, game_manager.segment_size)
    food.move_food_random(screen, snake)
    ...
    while game_manager.playing_game:
        ...
        if snake.colliding_with_body() or snake.colliding_with_walls(screen):
            ...
            # Make the head enlarge when eating food and then shrink back to normal size
...
def game_over_loop(screen, game_manager):
    ...
    game_over_menu = pg.transform.scale(game_over_menu, (game_manager.window_width, game_manager.window_height))
...
def display_text(lines_of_text, game_manager, screen, color, position):
    ...
    for line_number, line_text in enumerate(lines_of_text):
        ...
        screen.blit(
            text,
            (position[0], position[1] + line_number * line_height, game_manager.window_width - 10, game_manager.window_width - 10)
        )
